...
const HistoryView = ({ services, onSelect, initialFilter }: { services: WorkshopService[], onSelect: (id: string) => void, initialFilter?: string }) => {
  const [searchTerm, setSearchTerm] = useState('');
...
const StaffView = () => {
  const staff = [
    { id: 1, name: 'Ani', role: 'Kasir', status: 'Active', shifts: 'Pagi' },
    { id: 2, name: 'Bambang', role: 'Mekanik', status: 'Active', shifts: 'Sore' },
    { id: 3, name: 'Joko', role: 'Mekanik', status: 'Inactive', shifts: 'Pagi' },
  ];
  ...
  return (
      <div className="space-y-4 pb-20">
        <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm">
          ...
          <button className="w-full mt-6 py-4 border-2 border-dashed border-slate-100 rounded-2xl text-blue-600 font-black text-xs uppercase tracking-widest hover:bg-blue-50 transition-all">
...
export default function AppLayout() {
  ...
  const navItems = [
    ...
    { id: 'pos_history', label: 'Antrean', icon: Wrench, inBottom: true, roles: ['Mekanik'] },
            ...
            <AnimatePresence mode="wait">
              ...
              {!showPOSForm && activeTab === 'pos_history' && (
                  <motion.div key="pos-history" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                    ...
                    <HistoryView services={services} onSelect={setSelectedServiceId} initialFilter="In Progress" />
                  </motion.div>
              )}
